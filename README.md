...
The Context API natively solves the prop drilling problem in applications within it's on React Library.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Actions are types of methods containing data that gets sent to the reducer when the action is triggered.

The reducer then receives this action and any data it receives through it and handles the method given the type of case that action is.

The store receives that reducers method and updates the state within the store across the entire application.

The store is known as the single source of truth because it is immutable, predictable, and reliable on all factors of entire application state.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?

The redux-thunk package allows us to make asynchronous calls in our actions. This makes it possible for our actions be more specific and modularize the functionality of our application as well while still using redux.

4. What is your favorite state management system you've learned and this sprint? Please explain why!

Redux has been this big bad monster through my entire react experience. I've really tried mastering it but it never quite clicked in my head. It is very satisfying to be able to build semi-complex applications this entire week with redux. It's been extremely satisfying to finally best this monster technology, and for that reason, I have to say it is my favorite. Although I do see how simple Context is, I am just a bit put off on it's scaling capabilities, and for that reason, I'd prefer to learn and grow with something a bit more bulletproof.
